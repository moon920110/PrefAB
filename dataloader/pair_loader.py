...
class PairLoader(Dataset):
    def __init__(self, config):
        self.dataset, self.numeric_columns = AgainReader(config).prepare_sequential_ranknet_dataset()
        self.config = config
            ...
            for game in x['game'].unique():
                for player in x['player_id'].unique():
                    ...
                    for idx in range(5, len(player_data)):
                        ...
                        seq['game'] = self.config['game_numbering'][self.config['train']['genre']][game]
    ...
    def __getitem__(self, idx):
        img_data = self.x_img_pairs[idx]
        meta = self.x_meta_pairs[idx]
        y = self.y[idx]
        ...
        if self.sequential:
            with h5py.File(img_path, 'r') as f:
                ...
                compare_frames = torch.stack(
                    [self.transform(np.array(f[f'frames/{time_index}_{time_stamp}']).astype(np.float32))
            ...
            return compare_frames, torch.tensor(meta[:4]), main_frames, torch.tensor(meta[1:]), torch.tensor(y)

        else:
            with h5py.File(img_path, 'r') as f:
                frames = self.transform(np.array(f[f'frames/{time_indices}_{time_stamps}']))
