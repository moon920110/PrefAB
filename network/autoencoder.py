...
class AutoEncoder(nn.Module):
    def __init__(self):
        ...
        self.conv1 = nn.Conv2d(3, 16, 5, 1, dilation=2)
        self.max_pool = nn.MaxPool2d(2, 2)
        self.conv2 = nn.Conv2d(16, 8, 3, 2)
        self.conv3 = nn.Conv2d(8, 3, 3, 1)

        self.conv_t1 = nn.ConvTranspose2d(3, 8, 3, 1)
        self.conv_t2 = nn.ConvTranspose2d(16, 16, 3, 2, output_padding=1)
        self.upsample = nn.Upsample(scale_factor=2)
        self.conv_t3 = nn.ConvTranspose2d(32, 3, 5, 1, dilation=2)

    def forward(self, x):
        x1 = F.relu(self.conv1(x))
        x1_down = self.max_pool(x1)
        x2 = F.relu(self.conv2(x1_down))
        e = F.relu(self.conv3(x2))  # 3 x 75 x 115 = 25875
